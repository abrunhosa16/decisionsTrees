...
class DecisionTreeClassifier:
    ...
    def build_tree(self, dataset: pd.DataFrame, cur_depth: int = 0) -> Node:
        features, target = self.split_features_target(dataset)
        x = dataset[features] 
        y = dataset[target]
        ...
        if num_samples >= self.min_samples_split and cur_depth <= self.max_depth:
            best_split = self.get_best_split(dataset) #dict com feature, info_gain, k: k_dataset
    ...
    def get_best_split(self, dataset: pd.DataFrame) -> dict: #dict com feature, info_gain e k: k_dataset
    ...
    def fit(self, x: pd.DataFrame, y: pd.DataFrame) -> None: #função para construir a DT
